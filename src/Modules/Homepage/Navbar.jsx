        ...
        <Toolbar >
            ...
            <Grid container direction="row" justifyContent="space-between" alignItems="center">
                <Grid item xs={6} container spacing={1} direction="row" justifyContent="flex-start" alignItems="center">
                    ...
                    >
                        {languages.map(l => (
                            <MenuItem disabled key={l.id} value={l.id} onClick={() => { setLang(l.id); props.setAlert({ open: true, text: "Still not implemented, sorry :(", severity: "warning" }) }}>{l.id} {l.name}</MenuItem>
                        ))}
                    </Menu>
                    <Grid item>
                        <IconButton disabled size="small" style={{ borderRadius: '10px' }} onClick={props.changeLightMode}>
                ...
                <Grid item xs={2} container direction="row" justifyContent="center" alignItems="center" style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
                    <ButtonBase disableRipple style={{color:"#595f39"}}>
